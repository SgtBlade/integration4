...
const PostcardBackgrounds = (props) => {
  ...
  const handleClick = () => {
    if (props.postcard.background !== null) props.nextFunction();
  };
    ...
    <div className={style.container}>
      <div className={style.mainContent}>
        ...
        <ul className={style.postcardOptions}>
          {backgrounds.map((imageName, index) => {
            return (
              <li
                className={`${style.postcardOptions__option} ${
                  props.postcard.background === imageName ? style.selected : ""
                }`}
                key={index}
              >
                <img
                  ...
                  width={"261"}
                  height={"200"}
                  src={`/assets/postcards/${props.country}/${imageName}.png`}
                  alt={`Postkaart optie ${index}`}
                  className={style.postcardOptions__image}
                />
              </li>
            );
          })}
      ...
      <div className={style.nextButton}>
        <GeneralButton
          ...
          boxShadow={`0rem .5rem ${COLORS.green}`}
